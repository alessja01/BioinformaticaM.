...
Scrivere un MSA completo ,come Clustal, richiede migliaia di righe, ma il concetto chiave che differenzia l'allineamento progressivo da quello a coppie è la capacità di allineare un 
profilo contro un altro profilo. \\
...
\subsection{Ottimizzazioni Euristiche nel Calcolo delle Distanze}
\label{subsec:prog_heuristics}
Sebbene il calcolo formale della matrice delle distanze richieda un allineamento globale ottimale per ogni coppia di sequenze ($O(N^2 L^2)$), le implementazioni reali come ClustalW adottano euristiche per abbattere i tempi di calcolo nella prima fase. Invece di eseguire una programmazione dinamica completa (Needleman-Wunsch) per tutte le coppie, si utilizza spesso un approccio basato sul conteggio delle \textit{k-tuple} (o \textit{word counts}), simile a quello utilizzato nei motori di ricerca per database come FASTA o BLAST. 
L'algoritmo identifica brevi segmenti di lunghezza $k$ (es. $k=1$ o $2$ per le proteine) identici tra due sequenze e stima la distanza evolutiva basandosi sul numero di match trovati, penalizzando le differenze di lunghezza. Questo approccio riduce la complessità della prima fase a $O(N^2 L)$ o $O(N^2)$, rendendo l'algoritmo scalabile anche per centinaia di sequenze, pur introducendo un margine di approssimazione nella topologia iniziale dell'albero guida.

\subsection{Penalità dei Gap Posizione-Specifiche nei Profili}
\label{subsec:profile_gap_pen}
Una delle criticità maggiori nell'allineamento profilo-profilo è la determinazione del costo di apertura ed estensione dei gap. A differenza dell'allineamento a coppie, dove le penalità sono fisse (affine gap penalty), nell'allineamento progressivo moderno le penalità sono dinamiche e dipendono dal contesto del profilo. 
Software come ClustalW implementano regole \textit{position-specific}:
\begin{itemize}
    \item \textbf{Penalità ridotta} per l'apertura di un gap in posizioni dove sono già presenti gap (per favorire l'allineamento a blocchi).
    \item \textbf{Penalità aumentata} se il gap cade all'interno di regioni idrofobiche o strutturalmente conservate (presunti elementi di struttura secondaria come $\alpha$-eliche).
    \item \textbf{Penalità per residui specifici} (es. Glicina o Prolina) che favoriscono strutturalmente le curve e quindi le inserzioni/delezioni.
\end{itemize}
Queste regole tentano di mimare i vincoli strutturali biologici, compensando parzialmente la natura \textit{greedy} dell'algoritmo.